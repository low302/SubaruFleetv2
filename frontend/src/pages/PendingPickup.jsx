...
export default function PendingPickup() {
    ...
    const loadVehicles = async () => {
        try {
            ...
            setVehicles(data.filter(v => v.status === 'pending-pickup'));
        } catch (error) {
            console.error('Error loading pending pickup vehicles:', error);
        } finally {
            setLoading(false);
        }
    };
        ...
        <div className="p-6 lg:p-8">
            ...
            >
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <h1 className="text-2xl font-bold text-slate-100">Pending Pickup</h1>
                        <p className="text-slate-400 text-sm">
                            {filteredVehicles.length} vehicles awaiting pickup scheduling
                        </p>
                    </div>
                </div>
            </motion.div>
