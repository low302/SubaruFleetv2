...
export default function PickupScheduled() {
    ...
    const loadVehicles = async () => {
        try {
            ...
            setVehicles(data.filter(v => v.status === 'pickup-scheduled'));
        } catch (error) {
            console.error('Error loading scheduled pickup vehicles:', error);
        } finally {
            setLoading(false);
        }
    };
        ...
        <div className="p-6 lg:p-8">
            ...
            >
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <h1 className="text-2xl font-bold text-slate-100">Pickup Scheduled</h1>
                        <p className="text-slate-400 text-sm">
                            {filteredVehicles.length} vehicles with scheduled appointments
                        </p>
                    </div>
                </div>
            </motion.div>
            ...
            >
                <div className="relative">
                    ...
                    <input
                        type="text"
                        placeholder="Search by stock #, make, model, or customer..."
                        value={search}
                    ...
                    <table className="w-full">
                        <thead>
                            <tr className="bg-primary/10">
                                ...
                                <th className="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider text-slate-400">Pickup</th>
                            ...
                            {loading ? (
                                ...
                                filteredVehicles.map((vehicle, index) => (
                                    ...
                                    >
                                        ...
                                        <td className="px-4 py-3 text-sm text-slate-300">
                                            <div>{formatDate(vehicle.pickupDate)}</div>
                                            {vehicle.pickupTime && <div className="text-xs text-slate-500">{vehicle.pickupTime}</div>}
                                        </td>
